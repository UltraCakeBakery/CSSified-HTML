...
const attributeRegex = /([a-zA-Z0-9-_@:]*?)=["]([^"]*)/gm
    ...
    for ( let [ _, name, value ] of attributes )
    {
        ...
        for ( const prefix of prefixes )
        {
            if ( prefix[0] === '@' )
            {
                ...
                {
                    const prefixSplitted = prefix.substring(1).split('-')
                    const valueFromPrefixSplitted = prefixSplitted.pop() as string
                    entry.wrappingMediaQuery.conditions += `(${prefixSplitted.join('-')}: ${valueFromPrefixSplitted})`
                }
            }

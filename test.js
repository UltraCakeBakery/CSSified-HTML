...
const attributes = html.matchAll(/([a-zA-Z0-9-_:@%\[\]]*?)=['"]([^'"]*)/gm);
...
for (let [_, name, value] of attributes) {
    ...
    const property = variants.pop()
    
    if ( blacklist.indexOf(name) !== -1 ) continue
    if ( cssProperties.indexOf(property) === -1 ) continue

    let selector = ''
    
    
    // TODO: escape property name and value
    selector += `[${CSSEscape(name)}="${value}"]`
